...
impl AABB {
    ...
    pub fn merge(box1: &AABB, box2: &AABB) -> Self {
        let mut min = [f32::MAX; 3];
        let mut max = [f32::MIN; 3];
        box1.min.iter().zip(box2.min).enumerate().map(|(i, (x1, x2))| min[i] = x1.min(x2));
        box1.max.iter().zip(box2.max).enumerate().map(|(i, (x1, x2))| max[i] = x1.max(x2));
        Self {
            min,
            max,
        }
